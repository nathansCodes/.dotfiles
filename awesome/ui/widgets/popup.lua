...
function builder.create_input_popup(title, message, confirm_button_text, on_confirm, hide_input)
    ...
    local box = awful.popup {
        ...
        border_width = beautiful.popup_border_width or dpi(2),
...
function M.create_popup(args)
    if args.type == "input" then
        return builder.create_input_popup(args.title, args.message, args.confirm_button_text,
                                          args.on_confirm, args.hide_input)
    end
end
